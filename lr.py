...
def parse(produ, numm, string):
    ...
    prod.insert(0, "X->.E")
    ans.append("---------------------------------------------------------------")
    ans.append("Augmented Grammar")
    lis = ','.join([str(elem) for elem in prod])
    ans.append(lis)
        ...
        try:
            for j in dfa[i]:
                if not j.isupper() and j != '' and j != '.':
                    ind = term.index(j)
                    data[ind] = 'E' + str(dfa[i][j])
                    samp[term[ind]] = 'E' + str(dfa[i][j])
    ...
    # print(final_table)
    print("\n")
    dic["table"] = final_table
    dic["dfa"] = dfa
    dic["table_dic"] = table_dic
    # Parse String
    # print(dic["table"])
    if (numm == 0):
        #directory = os.path.dirname("pythonProject/")
        #if not os.path.exists(directory):
            #os.makedirs(directory)
    ...
    while True:
        try:
            ...
            if tab_i == 'Accept':
                ...
                if tab_i[0] == 'E' and not str(stack[-1]).isupper():
    ...
    try:
        ...
        dic["stack"] = "Invalid string entered!"
        return dic
    return dic